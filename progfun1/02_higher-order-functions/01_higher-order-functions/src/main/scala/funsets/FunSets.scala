...
object FunSets {
   ...
   */
  def union(s: Set, t: Set): Set = (item: Int) => contains(s, item) | contains(t, item)
  
  /**
   * Returns the intersection of the two given sets,
   * the set of all elements that are both in `s` and `t`.
   */
  def intersect(s: Set, t: Set): Set = (item: Int) => contains(s, item) & contains(t, item)
  
  /**
   * Returns the difference of the two given sets,
   * the set of all elements of `s` that are not in `t`.
   */
  def diff(s: Set, t: Set): Set = (item: Int) => contains(s, item) & !contains(t, item)

  /**
   * Returns the subset of `s` for which `p` holds.
   */
  def filter(s: Set, p: Int => Boolean): Set = (item: Int) => contains(s, item) & p(item)
